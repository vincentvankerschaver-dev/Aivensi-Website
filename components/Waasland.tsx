...
export function Waasland() {
  ...
  const [hubName, hx, hy] = TOWNS[0];
  return (
    <svg ref={ref} viewBox="0 0 600 380" aria-label="Kaart van het Waasland" style={{ width: '100%', maxWidth: 560, height: 'auto', display: 'block', justifySelf: 'end', overflow: 'visible', fontFamily: 'var(--font-mono)' }}>
      <g aria-hidden="true">
      ...
      {TOWNS.map(([name, x, y, km, anchor], i) => {
        const home = i === 0;
        return (
          <Link key={name} href="/contact" aria-label={home ? `${hubName} — thuisbasis` : `${name} — ${km} km`} style={{ outlineOffset: 6 }}>
